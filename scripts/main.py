        ...
        if hasattr(response.web_pages, 'value'):
            for page in response.web_pages.value:
                ...
                if page.url in output:
                    ...
                    c[page.url] = (c[page.url]*(output[page.url] - 1) + cosineSim(text,strip_tags(content)))/(output[page.url])
                else:
                    output[page.url] = 1
...
def main():
    ...
    if len(sys.argv) < 3:
        ...
        print "using the inputfile=sampleText.txt, outputfile=sampleOut.txt"
    else:
        inputFile = sys.argv[1]
        outputFile = sys.argv[2]
    if inputFile.endswith(".docx"):
        t = docxExtract(inputFile)
    else:
        t=open(inputFile,'r')
        if not t:
